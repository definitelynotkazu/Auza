...
do
            ...
            o.new('AutoUsePotions', function()
                while task.wait(0.5) do
                    ...
                    for _, potion in ipairs(selectedPotions) do
                        ...
                        local itemName, itemData = q.GetItemByName('potion', potionValue)
                        if not itemName or not itemData.am or itemData.am < 1 then continue end
                        Q:useItem(itemName, { use = 1 })
                    end
                end
            end):Start()
        end
    ...
    -- Boxes Dropdown
    local boxOptions = {}
    for boxName, boxData in pairs(m.Boxes) do
        table.insert(boxOptions, { Name = boxData.name, Layout = boxData.layoutOrder, Value = boxName })
    end
    table.sort(boxOptions, function(a, b) return a.Layout > b.Layout end)
    ...
    sections.Misc_Items:CreateDropdown({
        ...
        MultiSelection = true,
        Flag = "BoxesDropdown",
        Callback = function(selectedBoxes)
            print("Selected Boxes:", selectedBoxes)
        end
    })

    -- Auto Open Boxes Toggle
    sections.Misc_Items:CreateToggle({
        Name = "Enable Auto Open Boxes",
        CurrentValue = false,
        Flag = "AutoOpenBoxes",
        Callback = function(enabled)
            if not enabled then
            ...
            o.new('AutoOpenBoxes', function()
                while task.wait(0.5) do
                    local selectedBoxes = aj.Value
                    if l.GetDictionarySize(selectedBoxes) == 0 then continue end
                    for _, box in ipairs(selectedBoxes) do
                        local boxName, boxValue = box.Name, box.Value
                        local itemName, itemData = q.GetItemByName('box', boxValue)
                        if not itemName or not itemData.am or itemData.am < 1 then continue end
                        Q:useItem(itemName, { use = itemData.am })
                    end
                end
            end):Start()
        end
...
-- Miscellaneous Tab: Pet Combining Section
do
    -- Pet Tier Dropdown
    local tierOptions = {}
    for tier, tierData in pairs(m.Tiers) do
        local nextTier = m.Tiers[tier + 1]
        if not nextTier then continue end
        table.insert(tierOptions, { Name = string.format('%s -> %s', tostring(tierData.name), tostring(nextTier.name)), Value = tier, AttributeName = tierData.attributeName })
    end

    sections.Misc_Combining:CreateDropdown({
        Name = "Select Pet Tiers",
        Options = tierOptions,
        CurrentOption = {},
        MultiSelection = true,
        Flag = "UpgradeTierDropdown",
        Callback = function(selectedTiers)
            print("Selected Pet Tiers:", selectedTiers)
        end
    })

    -- Auto Upgrade Pets Toggle
    sections.Misc_Combining:CreateToggle({
        Name = "Enable Auto Upgrade Pets",
        CurrentValue = false,
        Flag = "AutoUpgradeTiers",
        Callback = function(enabled)
            if not enabled then
                o.TerminateByIndex'AutoUpgradeTiers'
                return
            end
            o.new('AutoUpgradeTiers', function()
                while task.wait(0.5) do
                    local selectedTiers = am.Value
                    if l.GetDictionarySize(selectedTiers) == 0 then continue end
                    for _, tier in ipairs(selectedTiers) do
                        local tierValue = tier.Value
                        if not m.Tiers[tierValue + 1] then continue end
                        local pets = q.GetPetsByTier(tierValue)
                        for petId, petData in pairs(pets) do
                            if petData:getLocked() then continue end
                            if petData:getAmount() < 5 then continue end
                            N:craft({ petId }, true)
                        end
                    end
                end
...
-- Miscellaneous Tab: Rewards Section
do
    -- Auto Claim Rewards Toggle
    sections.Misc_Misc:CreateToggle({
        Name = "Enable Auto Claim Rewards",
        CurrentValue = false,
        ...
        Callback = function(enabled)
            ...
            o.new('AutoClaimRewards', function()
                while task.wait(1) do
                    for rewardId, rewardData in pairs(m.PlaytimeRewards) do
                        if table.find(T.data.claimedPlaytimeRewards, rewardId) then continue end
                        if rewardData.required - T.data.sessionTime > 0 then continue end
                        if M:claimPlaytimeReward(rewardId) == "success" then
                            Rayfield:Notify({
                                Title = "Playtime Reward Claimed",
                                Content = string.format("Claimed playtime reward #%s.", tostring(rewardId)),
                                Duration = 1.5
                            })
                        end
                    end
                    if (T.data.dayReset - os.time() + 86400) <= 0 then
                        if M:claimDailyReward() == "success" then
                            Rayfield:Notify({
                                ...
                                Content = "Today's daily reward has been claimed!",
                                Duration = 1.5
                            })
                        end
                    end
                    for achievementClass, achievementData in pairs(m.Achievements) do
                        local nextAchievement, tier = q.GetNextAchievementByClass(achievementClass)
                        if not nextAchievement then continue end
                        if tier.amount > achievementData.getValue(T.data) then continue end
                        if M:claimAchievement(achievementClass) == "success" then
                            Rayfield:Notify({
                                Title = "Achievement Claimed",
                                Content = string.format("Claimed achievement %s (tier %s).", tostring(achievementClass), tostring(tier)),
                                Duration = 1.5
                            })
                        end
                    end
    ...
    -- Auto Claim Index Rewards Toggle
    sections.Misc_Misc:CreateToggle({
        Name = "Enable Auto Claim Index Rewards",
        CurrentValue = false,
        Flag = "AutoClaimIndexRewards",
        Callback = function(enabled)
            if not enabled then
                o.TerminateByIndex'AutoClaimIndexRewards'
                return
            end
            o.new('AutoClaimIndexRewards', function()
                while task.wait(1) do
                    for rewardId, rewardData in pairs(m.IndexRewards) do
                        if table.find(T.data.claimedIndexRewards, rewardId) then continue end
                        if rewardData.required > m.Util.indexUtils.countIndex(T.data, true) then continue end
                        if O:claimIndexReward(rewardId) == "success" then
                            Rayfield:Notify({
                                Title = "Index Reward Claimed",
                                Content = string.format("Claimed index reward #%s.", tostring(rewardId)),
                                Duration = 1.5
                            })
                        end
                    end
    ...
    sections.Misc_Misc:CreateToggle({
        ...
        Callback = function(enabled)
            ...
            o.new('AutoPrestigeToggle', function()
                while task.wait(1) do
                    local currentPrestige = (T.data.prestige or 0) + 1
                    local prestigeData = m.Prestiges[currentPrestige]
                    if not prestigeData then continue end
                    if prestigeData.required > T.data.prestigeXp then continue end
                    if P:claim() == "success" then
                        Rayfield:Notify({
                            Title = "Prestige Completed",
                            Content = string.format("Successfully prestiged to level %s.", tostring(currentPrestige)),
                            Duration = 1.5
                        })
                    end
                end
...
-- Settings Tab: Configuration Section
do
    -- Keybind Example: Toggle UI
    sections.Settings_Config:CreateKeybind({
        Name = "Toggle UI",
        CurrentKeybind = "RightControl",
        HoldToInteract = false,
        Flag = "ToggleUIKeybind",
        Callback = function(Keybind)
            Rayfield:Toggle({ Keybind = Keybind })
        end
    })

    -- Color Picker Example: Theme Color
    sections.Settings_Config:CreateColorPicker({
        Name = "Theme Color",
